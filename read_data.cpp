...
void read_data(string path, vector< vector<int> >& node2hyperedge, vector< vector<int> >& hyperedge2node, vector< unordered_set<int> >& hyperedge2node_set)
{
	...
	// Process node2hyperedge and hyperedge2node to make sure each vector is in increasing order
	for (auto& node2hyperedge_vec : node2hyperedge)
	{
		sort(node2hyperedge_vec.begin(), node2hyperedge_vec.end());
	}
	for (auto& hyperedge2node_vec : hyperedge2node)
	{
		sort(hyperedge2node_vec.begin(), hyperedge2node_vec.end());
	}
